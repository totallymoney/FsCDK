    ...
    testList
        ...
        [
            ...
            test "s3Bucket lifecycle helper functions" {
                ...
                let deleteVersionsRule = LifecycleRuleHelpers.deleteNonCurrentVersions 180 "delete-old-versions"
                Expect.equal deleteVersionsRule.Id "delete-old-versions" "Rule ID should be set"
                Expect.isTrue (deleteVersionsRule.Enabled.HasValue && deleteVersionsRule.Enabled.Value) "Rule should be enabled"
                Expect.isNotNull deleteVersionsRule.NoncurrentVersionExpiration "NoncurrentVersionExpiration should be set"
            }
            
            test "s3Bucket builder with custom construct ID" {
                let bucketResource = s3Bucket "test-bucket" { 
                    constructId "CustomBucketId"
                }
