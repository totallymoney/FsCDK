  ...
  <| testList
        ...
        [ test "fails when definition is missing" {
          ...
          test "defaults constructId to state machine name" {
              ...
              let sf = stepFunction "MyStateMachine" {
          ...
          test "StateMachineTypes helpers provide standard types" {
              Expect.equal StepFunctionHelpers.StateMachineTypes.standard StateMachineType.STANDARD "Standard type should match"
              Expect.equal StepFunctionHelpers.StateMachineTypes.express StateMachineType.EXPRESS "Express type should match"
          } ]
