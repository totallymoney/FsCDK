...
let vpc_dsl_tests =
    testList
        ...
        [ test "creates VPC with default settings" {
              ...
              Expect.equal (spec.Props.MaxAzs |> Option.ofNullable |> Option.defaultValue 0 |> int) 2 "Should default to 2 AZs for HA"
              Expect.equal (spec.Props.NatGateways |> Option.ofNullable |> Option.defaultValue 0 |> int) 1 "Should default to 1 NAT gateway"
              Expect.isTrue (spec.Props.EnableDnsHostnames |> Option.ofNullable |> Option.defaultValue false) "DNS hostnames should be enabled by default"
              Expect.isTrue (spec.Props.EnableDnsSupport |> Option.ofNullable |> Option.defaultValue false) "DNS support should be enabled by default"
          }
          ...
          test "creates VPC with custom settings" {
              ...
              Expect.equal (spec.Props.MaxAzs |> Option.ofNullable |> Option.defaultValue 0 |> int) 3 "Should use custom max AZs"
              Expect.equal (spec.Props.NatGateways |> Option.ofNullable |> Option.defaultValue 0 |> int) 2 "Should use custom NAT gateways"
          ...
          test "allows disabling DNS settings" {
              ...
              Expect.isFalse (spec.Props.EnableDnsHostnames |> Option.ofNullable |> Option.defaultValue true) "DNS hostnames should be disabled"
              Expect.isFalse (spec.Props.EnableDnsSupport |> Option.ofNullable |> Option.defaultValue true) "DNS support should be disabled"
          }
