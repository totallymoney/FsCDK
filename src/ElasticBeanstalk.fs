...
type ElasticBeanstalkApplicationBuilder(name: string) =
    ...
    member _.Combine(state1: ElasticBeanstalkApplicationConfig, state2: ElasticBeanstalkApplicationConfig) : ElasticBeanstalkApplicationConfig =
        { ApplicationName = state2.ApplicationName
          ConstructId = state2.ConstructId |> Option.orElse state1.ConstructId
          Description = state2.Description |> Option.orElse state1.Description }

    member inline x.For(config: ElasticBeanstalkApplicationConfig, [<InlineIfLambda>] f: unit -> ElasticBeanstalkApplicationConfig) : ElasticBeanstalkApplicationConfig =
        let newConfig = f ()
    ...
    member _.ConstructId(config: ElasticBeanstalkApplicationConfig, id: string) =
...
type ElasticBeanstalkEnvironmentBuilder(name: string) =
    ...
    member _.Combine(state1: ElasticBeanstalkEnvironmentConfig, state2: ElasticBeanstalkEnvironmentConfig) : ElasticBeanstalkEnvironmentConfig =
        { EnvironmentName = state2.EnvironmentName
          ...
          OptionSettings = if state2.OptionSettings.IsEmpty then state1.OptionSettings else state2.OptionSettings }

    member inline x.For(config: ElasticBeanstalkEnvironmentConfig, [<InlineIfLambda>] f: unit -> ElasticBeanstalkEnvironmentConfig) : ElasticBeanstalkEnvironmentConfig =
        let newConfig = f ()
        x.Combine(config, newConfig)
    ...
    member _.OptionSettings(config: ElasticBeanstalkEnvironmentConfig, settings: CfnEnvironment.IOptionSettingProperty list) =
        { config with OptionSettings = settings }
