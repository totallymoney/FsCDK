...
type UserBuilder() =
    ...
    member _.Combine(state1: UserConfig, state2: UserConfig) : UserConfig =
        { ConstructId = state2.ConstructId |> Option.orElse state1.ConstructId
          Groups = Seq.append state1.Groups state2.Groups
    ...
    member _.Groups(config: UserConfig, group: IGroup seq) =
        { config with
            Groups = Seq.append config.Groups group }
    ...
    member _.ManagedPolicies(config: UserConfig, policy: IManagedPolicy seq) =
        { config with
            ManagedPolicies = Seq.append config.ManagedPolicies policy }
...
type PolicyBuilder(name: string) =
    ...
    member _.Combine(state1: PolicyConfig, state2: PolicyConfig) : PolicyConfig =
        { ConstructId = state2.ConstructId |> Option.orElse state1.ConstructId
          ...
          Roles = Seq.append state1.Roles state2.Roles
          Statements = Seq.append state1.Statements state2.Statements
          Users = Seq.append state1.Users state2.Users }
    ...
    member _.Statements(config: PolicyConfig, statement: PolicyStatement seq) =
        { config with
            Statements = Seq.append config.Statements statement }
    ...
    /// </code>
    [<CustomOperation("groups")>]
    member _.Groups(config: PolicyConfig, group: IGroup seq) =
        { config with
            Groups = Seq.append config.Groups group }
    ...
    member _.Users(config: PolicyConfig, user: IUser seq) =
        { config with
            Users = Seq.append config.Users user }
...
type PolicyDocumentBuilder() =
    ...
    member _.Combine(a: PolicyDocumentConfig, b: PolicyDocumentConfig) : PolicyDocumentConfig =
        { AssignSids = a.AssignSids |> Option.orElse b.AssignSids
          ...
          Statements = a.Statements |> Seq.append b.Statements }
    ...
    member _.Run(config: PolicyDocumentConfig) : PolicyDocument =
        ...
        if not (Seq.isEmpty config.Statements) then
            config.Statements
            |> Seq.iter (fun stmt ->
                if props.Statements = null then
                    props.Statements <- [| stmt |]
                else
                    props.Statements <- Array.append props.Statements [| stmt |])

        let policyDoc = PolicyDocument(props)
        policyDoc
    ...
    /// </code>
    [<CustomOperation("statements")>]
    member _.Statements(config: PolicyDocumentConfig, statement: PolicyStatement seq) =
        { config with
            Statements = Seq.append config.Statements statement }
...
type ManagedPolicyBuilder(name: string) =
    ...
    member _.Combine(a: ManagedPolicyConfig, b: ManagedPolicyConfig) : ManagedPolicyConfig =
        { Description = a.Description |> Option.orElse b.Description
          ...
          Groups = Seq.append a.Groups b.Groups
          PolicyName = a.PolicyName
          Path = a.Path |> Option.orElse b.Path
          Roles = Seq.append a.Roles b.Roles
          Statements = Seq.append a.Statements b.Statements
          ManagedPolicyName = a.ManagedPolicyName |> Option.orElse b.ManagedPolicyName
          Users = Seq.append a.Users b.Users
    ...
    /// </code>
    [<CustomOperation("statements")>]
    member _.Statements(config: ManagedPolicyConfig, statement: PolicyStatement seq) =
        { config with
            Statements = Seq.append config.Statements statement }
    ...
    /// </code>
    [<CustomOperation("groups")>]
    member _.Groups(config: ManagedPolicyConfig, group: IGroup seq) =
        { config with
            Groups = Seq.append config.Groups group }
    ...
    /// </code>
    [<CustomOperation("users")>]
    member _.Users(config: ManagedPolicyConfig, user: IUser seq) =
        { config with
            Users = Seq.append config.Users user }

    /// <summary>Attaches the policy to a role.</summary>
    /// <param name="config">The current managed policy configuration.</param>
    /// <param name="role">The IAM role to attach the policy to.</param>
    /// <code lang="fsharp">
    /// managedPolicy "MyPolicy" {
    ///     roles [
    ///       myRole1
    ...
    member _.Roles(config: ManagedPolicyConfig, role: IRole seq) =
        { config with
            Roles = Seq.append config.Roles role }
