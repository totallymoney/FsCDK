...
module Observability =
    ...
    let createLambdaErrorAlarm (alarmName: string) (functionName: string) (threshold: float option) (evaluationPeriods: int option) =
    ...
    let createRDSCpuAlarm (alarmName: string) (dbInstanceIdentifier: string) (threshold: float option) (evaluationPeriods: int option) =
        let props = AlarmProps()
        props.AlarmName <- alarmName
    ...
    /// <param name="evaluationPeriods">Number of periods to evaluate (default: 2)</param>
    let createALB5xxAlarm (alarmName: string) (loadBalancerFullName: string) (threshold: float option) (evaluationPeriods: int option) =
        let props = AlarmProps()
        props.AlarmName <- alarmName
    ...
    let createAlarmFromTemplate (alarmName: string) (template: AlarmTemplate) =
        ...
        | LambdaErrors (functionName, threshold) ->
            createLambdaErrorAlarm alarmName functionName threshold None
        | RDSHighCpu (dbIdentifier, threshold) ->
            createRDSCpuAlarm alarmName dbIdentifier threshold None
        | ALB5xxErrors (loadBalancerName, threshold) ->
            createALB5xxAlarm alarmName loadBalancerName threshold None
