...
type S3BucketBuilder(name: string) =
    ...
    member _.Encryption(config: S3BucketConfig, value: BucketEncryption) = { config with Encryption = Some value }
    ...
    member _.EnforceSSL(config: S3BucketConfig, value: bool) = { config with EnforceSSL = Some value }

    [<CustomOperation("versioned")>]
    member _.Versioned(config: S3BucketConfig, value: bool) = { config with Versioned = Some value }
