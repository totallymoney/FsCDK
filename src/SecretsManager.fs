...
type SecretConfig =
    { SecretName: string
      ...
      GenerateSecretString: ISecretStringGenerator option }
...
type SecretBuilder(name: string) =
    member _.Yield _ : SecretConfig =
        { SecretName = name
          ConstructId = None
          Description = None
          EncryptionKey = None
    ...
    member _.Combine(state1: SecretConfig, state2: SecretConfig) : SecretConfig =
    ...
    member inline x.For(config: SecretConfig, [<InlineIfLambda>] f: unit -> SecretConfig) : SecretConfig =
        let newConfig = f ()
        x.Combine(config, newConfig)

    member _.Run(config: SecretConfig) : SecretSpec =
    ...
    member _.ConstructId(config: SecretConfig, id: string) = { config with ConstructId = Some id }

    [<CustomOperation("description")>]
    member _.Description(config: SecretConfig, description: string) =
        { config with
            Description = Some description }

    [<CustomOperation("encryptionKey")>]
    member _.EncryptionKey(config: SecretConfig, key: IKey) =
        { config with
            EncryptionKey = Some(key) }

    [<CustomOperation("removalPolicy")>]
    member _.RemovalPolicy(config: SecretConfig, policy: RemovalPolicy) =
        { config with
            RemovalPolicy = Some policy }

    [<CustomOperation("secretStringValue")>]
    member _.SecretStringValue(config: SecretConfig, value: SecretValue) =
        { config with
            SecretStringValue = Some value }

    [<CustomOperation("generateSecretString")>]
    member _.GenerateSecretString(config: SecretConfig, generator: SecretStringGenerator) =
        { config with
...
module SecretStringGenerator =
    /// <summary>
    /// Creates a secret string generator for a random password
    /// </summary>
    let password (length: int) (excludeCharacters: string option) =
    ...
    /// </summary>
    let jsonSecret (secretStringTemplate: string) (generateStringKey: string) =
        let gen = SecretStringGenerator()
...
module SecretsManagerBuilders =
    ...
    /// </summary>
    let secret name = SecretBuilder(name)
