...
let config = Environment.GetEnvironmentVariable("CONFIG") |> fun v -> if String.IsNullOrWhiteSpace(v) then "Release" else v
...
pipeline "ci" {
  ...
  stage "test" {
    run (sprintf "dotnet test tests -c %s --no-build" config)
  }
